...
#define TARGET_DEFAULT_1 0
#define TARGET_DEFAULT_2 1
...
void set_args(int argc, char* argv[], int& qreg_size, int& target_idx_1, int& target_idx_2, int& ngates);
int set_verbose();
...
int main(int argc, char *argv[]) {
  ...
  int target_idx_1 = TARGET_DEFAULT_1;
  int target_idx_2 = TARGET_DEFAULT_2;
  int ngates = NGATES_DEFAULT;

  set_args(argc, argv, qreg_size, target_idx_1, target_idx_2, ngates);
  ...
  // Test SWAPs (or another 2-qubit gate)
  for (int i = 0; i < ngates; i++)
    swapGate(qureg, target_idx_1, target_idx_2);
    // controlledPhaseShift(qureg, target_idx_1, target_idx_2, 2 * M_PI / 16);
...
void set_args(int argc, char* argv[], int& qreg_size, int& target_idx_1, int& target_idx_2, int& ngates) {
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "-q") {
      ...
      target_idx_1 = atoi(argv[++i]);
    } else if (arg == "-t2") {
      target_idx_2 = atoi(argv[++i]);
    } else if (arg == "-n") {
      ngates = atoi(argv[++i]);
    } else {
